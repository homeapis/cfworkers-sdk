...
const middleware = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    /**
...
const parseJwtClaims = (request: IRequest): edge.jwt.JwtData => {
    let jwt = request.headers.get("authorization")?.substring(7) as string || request.query.access_token?.toString() as string;
...
const verifyAuthorizationClaims = (request: IRequest, scope: ApiScope[]) => {
    ...
    scope.map((requiredScope: ApiScope) => {
        ...
        if (!jwtScope.includes(requiredScope)) {
            return edge.handler.throwError(request, ["Unauthorized", `This endpoint requires a scope that is not granted by your access token.`, {
                scope: {
                    required: scope,
                    provided: jwtScope,
                }
...
export { middleware, getPublicSigningKey, verifyAuth, onValidAuthResponse, parseJwtClaims, verifyAuthorizationClaims }