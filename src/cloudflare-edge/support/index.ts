...
const getSupportThread = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        // get ticket from DB
        const ticket: D1Ticket = await env.SUPPORT_DB.prepare('SELECT * FROM tickets WHERE ticket_id = ?1').bind(ticket_id).first();

        // when ticket exists
        if (!ticket) return homeapis.edge.handler.throwError(request, ["Error", "NoTicketWithID"], 404);
...
const getSupportThreadsForAccount = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        const authuser = await homeapis.edge.video.readOrkaJwtPayload(request, env);
        if (!authuser) return homeapis.edge.handler.throwError(request, ["Preview_CFAuthError", "NoAccessProfileFound"], 401)
        ...
        // get ticket from DB
        const tickets = await env.SUPPORT_DB.prepare('SELECT * FROM tickets WHERE customer_email = ?1').bind(ticket_email).all();

        // when ticket exists
        if (!tickets.success) return homeapis.edge.handler.throwError(request, ["Error", "NoTicketWithID"], 404);
