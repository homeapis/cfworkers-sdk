...
class VideoDataFormatter {
    ...
    getFormattedVideoData () {
        let mainVideoFeedCurator: { id: string; title: string; short_id: string; master_playlist_type: string; channel_id: string; video_length: BigInt | null; created_at: string; updated_at: string | null; owner: string; storyboard: { snapshot_count: number; feature_url: string; feature_url_optimized: string; formats: string[]; }; description: string; adaptive: boolean; enable_downloads: boolean; algorithm: { sorting_type: string; position: number; is_workers_ai: boolean; is_inference: boolean; score: number; }; }[] = [];
...
const getRandomOrkaEntries = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        const db_statement: D1Result<D1OrkaCloudVideo> = await env.ORKA_VIDEO_DB.prepare('SELECT * FROM videos WHERE RANDOM() LIMIT 10').all();
