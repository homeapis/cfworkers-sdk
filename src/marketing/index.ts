...
class MarketingCustomer {
  ...
  async getProfile() {
    // Query DB for data
    const mkt_profile: MarketingEmailD1 = await this.env.MARKETING_DB.prepare(
      'SELECT * FROM email_collection WHERE email_hash_sha256 = ?1'
    )
...
const collectPostEmailAddress = async (request: IRequest, env: Env) => {
  ...
  if (!rPayload?.mkt_campaign || !rPayload.mkt_channel || !rPayload.mkt_medium || !rPayload.email) {
    return edge.handler.throwError(request, ['MarketingDataNotSpecified'], 500);
  }

  let query: MarketingEmailQuery = rPayload;

  let em_hash = await edge.auth.createSha256HashHex(query.email);
  let now = Date.now();
  ...
  const emailExists: MarketingEmailD1 = await env.MARKETING_DB.prepare(
    'SELECT * FROM email_collection WHERE email_hash_sha256 = ?1'
  )
  ...
  if (emailExists && emailExists?.email) {
    // email exists
    return edge.handler.throwError(request, ['MktError', { email: em_hash }], 401);
  }
  ...
  try {
    ...
    return edge.handler.throwError(request, ['com.homeapis.schemas/err', 'cf/D1Error', err.message, data], 200);
  }
...
const getEmailRegistrationData = async (request: IRequest, env: Env) => {
  ...
  if (!profile) return edge.handler.throwError(request, ["Profile doesn't exist."], 404);
