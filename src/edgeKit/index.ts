...
export function throwError(
  ...
  return createResponse(req, error_body, status, {
...
function defaultApiResponseHeaders(request: IRequest, headers?: HeadersInit) {
  return {
    ...
    'X-BSS-Playback-Region-Scheme':
      `${(request.cf?.colo || '').toString()}.${(request.cf?.country || '').toString()}.edge.blkdwrf.net`.toLowerCase(),
...
export function createResponse(
  ...
  if (request.query.debug !== undefined) {
    return new Response(JSON.stringify(response, null, 2), {
      ...
      headers: defaultApiResponseHeaders(request, {
        'Content-Language': 'en-US',
        'X-HomeAPIs-Neat-Debug': 'Neat-UI'
      })
    });
  } else {
