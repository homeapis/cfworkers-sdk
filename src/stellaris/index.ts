...
import { homeapis } from "..";
import { platform } from "../d";
...
const searchRoms = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        if (!clientPayload || !clientPayload.query || !clientPayload.limit) return homeapis.edge.handler.throwError(request, "PostErr", 500)

        // Establish connection to the DB
        const db_statement: D1Result<GameRom> = await env.STELLARIS.prepare('SELECT * FROM roms WHERE title LIKE ?1 LIMIT ?2').bind(`%${clientPayload.query}%`, clientPayload.limit).all();

        if (!db_statement.success) {
            response = homeapis.edge.handler.throwError(request, 'DBWrongQuery', 500);
        } else {
...
const searchRomsByGet = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        if (!clientPayload || !clientPayload.query || !clientPayload.limit) return homeapis.edge.handler.throwError(request, "UnspecifiedParams", 403)
...
const getGameRoms = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        // start counting from the last shown entry +1
        const last_prev_id = request.query.page || 0;
        ...
        const DBErr = new platform.SupportResource(request, 101, { message: "D1 Data could not be read." })
        if (!roms.success || !roms.results) {
            ...
            response = homeapis.edge.handler.createResponse(request, {
                ...
                metadata: {
                    ...
                    last_item: last_id,
                    next_page_url: `https://api-edge.cf.lavndr.xyz/client/v3/stellaris/archive/roms?limit=${actual_limit}&page=${page + 1}`,
                }
            })
        }
...
const getGameRomBySha256 = async (request: IRequest, env: Env, ctx: ExecutionContext) => {
    ...
    if (!response) {
        ...
        // get games
        const rom: GameRom | null = await env.STELLARIS.prepare('SELECT * FROM roms WHERE sha256 = ?1').bind(rom_sha256).first();
