...
const validateJwt = async (req: IRequest, env: Env) => {
  ...
  const isValid = await edge.jwt.verify(token, env.ORKA_DEMO_SIGNING_KEY); // false
...
const readOrkaJwtPayload = async (req: IRequest, env: Env) => {
  const authHeader = req.headers.get('authorization');
  const token = authHeader?.substring(7) || req.query.access_token?.toString();
  ...
  return edge.jwt.decode(token);
};
export const signVideoPlaybackURL = async (videoId: string, time: number, env: Env) => {
...
const generateUserToken = async (request: IRequest, env: Env) => {
  ...
  const authuser: CFAccessAuthProps | null = await edge.auth.getAccessUserAccountProps(request);
...
const unwrap = async (request: IRequest, env: Env) => {
  ...
  if (signature !== credentials.hmac || credentials.exp == undefined) {
    return edge.handler.throwError(
      request,
      ["This video URL isn't signed properly or the signature was revoked.", 'InvalidHmac'],
      403
    );
  }
  ...
  if (Date.now() / 1000 > parseInt(credentials.exp.toString()) + expTimestamp) {
    return edge.handler.throwError(
      request,
      [`URL expired at ${new Date((parseInt(credentials.exp.toString()) + expTimestamp) * 1000)}`, 'InvalidTimestamp'],
      403
    );
  }
...
const fastlyPlayback = async (request: IRequest, env: Env) => {
  ...
  const prodDisable = new platform.SupportResource(request, 999, 501, {
    ...
    visibility: 'PRIVATE',
    message:
      'HMAC-only access URIs are reserved for public videos. Please use a newer endpoint that supports IAM access tokens.'
  });
...
const getVideoData = async (request: IRequest, env: Env) => {
  ...
  const data: D1OrkaCloudVideo | null = await env.ORKA_VIDEO_DB.prepare('SELECT * FROM videos WHERE id = ?1')
    .bind(video_id)
    .first();

  if (!data) {
    return new platform.SupportResource(request, 'D1_ERROR').throwError();
  }
...
const getVideoDataUsingShortId = async (request: IRequest, env: Env) => {
  ...
  const data: D1OrkaCloudVideo | null = await env.ORKA_VIDEO_DB.prepare('SELECT * FROM videos WHERE short_id = ?1')
    .bind(video_id)
    .first();
