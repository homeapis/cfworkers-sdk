...
import { AccountRegistrationBody, UserProfile, RequestCredentials } from './types';
import Env from '../env';
import * as tokens from './token-issuance';
import { edge } from '../..';

const retrieveAccessIdentity = async (request: IRequest) => {
  let cloudflare_access_header = request.headers.get('cf-access-jwt-assertion');

  if (!cloudflare_access_header) {
    return edge.handler.throwError(request, 'CannotReadAccessJwt', 401);
  }

  return edge.handler.createResponse(
    request,
    {
      decoded_jwt: edge.jwt.decode(cloudflare_access_header),
      is_verified: false
    },
    200,
    { headers: { 'X-HomeAPIs-Secure-IdP': 'ACTIVE' } }
  );
};

// Preferred method for accessing logged-in user props
const getAccessUserAccountProps = async (request: IRequest) => {
  let cloudflare_access_header = request.headers.get('cf-access-jwt-assertion');
  ...
  const user_profile: any = edge.jwt.decode(cloudflare_access_header);
  return user_profile;
};

const validateCloudflareAccessJwt = async (request: IRequest) => {
  let cloudflare_access_header = request.headers.get('cf-access-jwt-assertion');
  ...
  let jwt = edge.jwt.decode(cloudflare_access_header);

  return edge.handler.createResponse(request, { success: true, jwt }, 200);
};
 ...
 * @returns
 */
const accessMiddleware = async (request: IRequest) => {
  let cloudflare_access_header = request.headers.get('cf-access-jwt-assertion');

  if (!cloudflare_access_header) {
    return edge.handler.throwError(
      request,
      ['CannotReadAccessJwt', 'https://support.homeapis.com/platform/streaming/videos/get-video'],
      401
    );
  }
  ...
  if (is_unauthorized_origin) {
    return edge.handler.throwError(request, 'UnauthorizedEdgeDomain', 401);
  }

  return;
};

export const generateUserAccess = async (request: IRequest) => {
  let cloudflare_access_header = request.headers.get('cf-access-jwt-assertion');
...
const checkAuthValidity = async (email: string, password: string, env: Env) => {
  ...
  const accountQuery: UserProfile | null = await env.HOMEAPIS_USERS_DB.prepare(
...
const refreshUserJwt = async (req: IRequest) => {
  return edge.handler.throwError(req, 'Unavailable');
};
...
const signInWithEmailAndPassword = async (req: IRequest, env: Env) => {
  ...
  if (!edgeAuthenticationStatus) {
    let errorAuthResponse = {
      success: false,
      errors: [
        'This endpoint requires valid credentials in order to issue a JWT. See https://support.homeapis.com/platform/identity'
      ]
    };
  ...
  // Issue 2H JWT when authentication is valid
    ...
    const avatar = await edge.sonar.createAuthorizedUrl(
      edgeAuthenticationStatus._id,
      edgeAuthenticationStatus.avatar,
      env
    );
    ...
    // sign the token
    const token = await edge.jwt.sign(payload, env.HOMEAPIS_OPENSSL_JWT_SECRET);
    // Sign a refresh-only token that lasts 7 days
    const refreshToken = await edge.jwt.sign(
      {
        ...payload,
        refresh: true,
        exp: Math.floor(Date.now() / 1000 + 7 * 24 * 60 * 60)
      },
      env.HOMEAPIS_OPENSSL_JWT_SECRET
    );
...
const verifyJwtValidy = async (req: IRequest, env: Env) => {
  ...
  const isValid = await edge.jwt.verify(token, env.HOMEAPIS_OPENSSL_JWT_SECRET); // false
  ...
  // Decoding token
  const payload = edge.jwt.decode(token);
...
const handleAccountRegistration = async (account: any, env: Env): Promise<any> => {
  ...
  // Register link
  const registrationAttemp = await env.HOMEAPIS_USERS_DB.prepare(
    'INSERT INTO users (_id, created_at, name, username, email, password) VALUES (?1, ?2, ?3, ?4, ?5, ?6)'
  )
...
const createUserWithEmailAndPassword = async (req: IRequest, env: Env) => {
  ...
  if (!dbResponse) {
    return edge.handler.throwError(
      req,
      'User registration with internal systems failed, check request payload. See requirements: https://staging.docs.homeapis.com/video/endpoints',
      500
    );
  } else {
    // Respond with user profile
    let auth_response = { ...accountRegistrationData, password: undefined };
    return edge.handler.createResponse(req, auth_response, 200);
  }
};
...
const middleware = async (req: IRequest, env: Env) => {
  ...
  if (!token) return edge.handler.throwError(req, 'InvalidAuthenticationToken', 401);

  const is_valid = await edge.jwt.verify(token, env.HOMEAPIS_OPENSSL_JWT_SECRET);
  const is_refresh = await edge.jwt.decode(token).payload.refresh;

  // handle invalid token error
  if (!is_valid) {
    return edge.handler.throwError(req, 'InvalidAuthenticationToken', 401);
  }

  // check for refresh token
  if (is_refresh) {
    return edge.handler.throwError(req, 'IllegalRefreshToken', 403);
  }
...
const getCurrentUserProfile = async (req: IRequest, env: Env) => {
  ...
  if (!bearerToken) {
    return edge.handler.throwError(req, 'InvalidAuthenticationToken', 403);
  }

  const isValid = await edge.jwt.verify(bearerToken, env.HOMEAPIS_OPENSSL_JWT_SECRET); // false

  // Send qualified response
  if (!isValid) return edge.handler.throwError(req, 'InvalidAuthenticationToken', 403);

  // Respond with data stored in JWT
  let decodedPayload = edge.jwt.decode(bearerToken);
  return edge.handler.createResponse(req, decodedPayload.payload.user_metadata);
};
...
async function getPublicUserProfile(req: IRequest, env: Env, ctx: ExecutionContext) {
  ...
  if (!response) {
    ...
    let profile: UserProfile | null = await env.HOMEAPIS_USERS_DB.prepare('SELECT * FROM users WHERE username = ?1')
      .bind(username)
      .first();

    if (!profile) {
      response = edge.handler.throwError(req, 'ProfileDoesNotExist', 404);
    } else {
      let db_profile = {
        success: true,
        id: profile._id,
        ...
        picture: await edge.sonar.createAuthorizedUrl(profile._id, profile.avatar, env),
        joined: new Date(profile.created_at).toJSON()
      };

      response = edge.handler.createResponse(req, db_profile, 200);
    }
