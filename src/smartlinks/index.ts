...
import { auth0, edge, platform } from '..';

// define allowed patterns
const urlRegex =
  /^(https?|ftps?):\/\/(([a-z\d]([a-z\d-]*[a-z\d])?\.)+[a-z]{2,}|localhost)(\/[-a-z\d%_.~+]*)*(\?[;&a-z\d%_.~+=-]*)?(\#[-a-z\d_]*)?$/i;
...
const createSmartlink = async (request: IRequest, env: Env) => {
  ...
  if (!urlSlugRegex.test(link.shortId)) {
    return Response.json({
      success: false,
      errors: ["The requested regular expression is denied as per your organization's policy."]
    });
  }
  let isAvailable: boolean = true;
  const availabilityCheck: DbShortlink | null = await env.campaignsDB
    .prepare('SELECT * FROM Smartlinks WHERE shortId = ?1')
    .bind(link.shortId)
    .first();
  let shortlink: D1Result<ClientShortlink> = await env.campaignsDB
    .prepare('INSERT INTO Smartlinks (id, destination, shortId, createdAt, accountUid) VALUES (?1, ?2, ?3, ?4, ?5)')
    .bind(link.id, link.destination, link.shortId, link.createdAt, link.accountUid)
    .run();
  if (availabilityCheck?.shortId !== null) {
    // edit short prop
    shortlink = await env.campaignsDB
      .prepare('INSERT INTO Smartlinks (id, destination, shortId, createdAt, accountUid) VALUES (?1, ?2, ?3, ?4, ?5)')
      .bind(link.id, link.destination, link.id, link.createdAt, link.accountUid)
      .run();
    // set to false
    isAvailable = false;
  }
...
const listSmartlinks = async (request: IRequest, env: Env) => {
  ...
  // list links
  const { success, results }: D1Result<DbShortlink> = await env.campaignsDB
    .prepare('SELECT * FROM Smartlinks WHERE accountUid = ?1 LIMIT 50')
    .bind(sub)
    .all();
  if (!success) {
    let err = new platform.SupportResource(request, 'D1_ERROR', 401);
...
const followSmartlink = async (request: IRequest, env: Env) => {
  ...
  // get shortlink
  const link: DbShortlink | null = await env.campaignsDB
    .prepare('SELECT * FROM Smartlinks WHERE shortId = ?1')
    .bind(shortId)
    .first();
  if (!link) {
...
export { createSmartlink, listSmartlinks, followSmartlink, deleteSmartlink };
